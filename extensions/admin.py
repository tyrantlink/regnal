	...
	async def slash_admin_purge(self,ctx:ApplicationContext,messages:int=None,message_id:str=None) -> None:
		...
		if messages:
			try: purged = await ctx.channel.purge(limit=messages,reason=f'{ctx.author.name} used purge command')
				...
				await ctx.response.send_message(embed=Embed(title='PERMISSION ERROR',description='i am missing either `Manage Messages` or `Read Message History`\nbe sure to check both my integration role and channel overrides',color=0xff6969),
				ephemeral=await self.client.hide(ctx))
				return
		if message_id:
			try: message = Object(message_id)
			except TypeError:
				await ctx.response.send_message(embed=Embed(title='ERROR',description='invalid message_id given',color=0xff6969),
				ephemeral=await self.client.hide(ctx))
				return
			try: purged = await ctx.channel.purge(limit=None,after=message.created_at,reason=f'{ctx.author.name} used purge command')
				...
				ephemeral=await self.client.hide(ctx))
				return
		await ctx.response.send_message(embed=Embed(title=f'successfully purged {len(purged)} messages',color=await self.client.embed_color(ctx)),ephemeral=await self.client.hide(ctx))
