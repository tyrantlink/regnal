...
class ConfigOptionTypeHandler(ConfigOptionTypeHint):
	...
	async def handle_string(self) -> None:
		...
		self.get_item('select_string').options = [
			SelectOption(
				...
				value = option.value)
			for option in self.option.attrs.options]
	...
	async def _handle_mentionable(self,mode:str,multi:bool) -> None:
		...
		if self.option.attrs.max_value: self.get_item(f'select_{mode}').max_values = self.option.attrs.max_value
		if self.option.attrs.min_value: self.get_item(f'select_{mode}').min_values = self.option.attrs.min_value
		if multi: self.add_items(self.button_add,self.button_remove)
