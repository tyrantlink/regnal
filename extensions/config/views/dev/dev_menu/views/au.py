...
from discord import Interaction,Embed,SelectOption,InputTextStyle,Member
from utils.classes import EmptyView,CustomModal,AutoResponse
from client import Client


class au_view(EmptyView):
	def __init__(self,back_view:EmptyView,user:Member,client:Client,embed:Embed,au:dict) -> None:
	...
	def set_au_reload_flag(self) -> None:
		...
		if 'base' not in self.client.flags.get('RELOAD_AU'): self.client.flags['RELOAD_AU'].append('base')
	...
	def page(self,value) -> None:
		...
		match value:
			case 'main':
				self.add_items(self.au_select,self.back_button,self.new_button,self.previous_button,self.next_button)
				self.update_select()
			case 'new':
				self.add_items(self.method_select,self.limit_user_select,self.back_button,self.set_button,self.regex_button,self.nsfw_button,self.case_sensitive_button,self.file_button,self.alt_responses_button,self.followups_button,self.save_button)
				if self.selected_au:
	...
	def embed_au(self):
		...
		self.embed.add_field(name='file',value=self.selected_au.file)
	...
	async def edit_button(self,button:Button,interaction:Interaction) -> None:
		...
		self.get_item('file_button').style = 3 if self.selected_au.file else 4
		await interaction.response.edit_message(embed=self.embed,view=self)
	...
	async def remove_button(self,button:Button,interaction:Interaction) -> None:
		if self.remove_confirmed:
			...
			await self.client.db.inf('/reg/nal').auto_responses.unset([self.selected_au.trigger.replace('.','\.')])
			await self.client.log.info(f'{self.user.name} modified base auto responses',
				author=self.user.id,
	...
	async def set_button(self,button:Button,interaction:Interaction) -> None:
		modal = CustomModal(self,'add auto response',[
			InputText(label='limit guild',placeholder='guild id',min_length=0,max_length=30,style=InputTextStyle.short,required=False,value=self.selected_au.guild),
		...
		await modal.wait()
		if modal.children[1].value.startswith('$') or '.' in modal.children[1].value:
			...
			return
		self.selected_au.guild = modal.children[0].value if modal.children[0].value else None
		self.selected_au.trigger = modal.children[1].value
		self.selected_au.response = modal.children[2].value if modal.children[2].value else None
		self.embed_au()
	...
	async def save_button(self,button:Button,interaction:Interaction) -> None:
		...
		await self.client.db.inf('/reg/nal').auto_responses.write(self.selected_au.to_dict(False),[self.selected_au.trigger])
		await self.client.log.info(f'{self.user.name} modified base auto responses',
			author=self.user.id,
			trigger=self.selected_au.trigger)
		self.set_au_reload_flag()
