...
class EditLogEmbed(Embed):
    def __init__(self, after: Message, before: Message = None) -> None:
        ...
        match time_diff:
            case _ if time_diff < 1:
                diff_string = f'{time_diff*1000:.2f} milliseconds'
            case _ if time_diff < 60:
                diff_string = f'{time_diff:.2f} seconds'
            case _ if time_diff < 3600:
                diff_string = f'{time_diff/60:.2f} minutes'
            case _ if time_diff < 86400:
                diff_string = f'{time_diff/3600:.2f} hours'
            case _:
                diff_string = f'{time_diff/86400:.2f} days'
...
class DeleteLogEmbedFromID(Embed):
    def __init__(self, message_id: int, channel_id: int, author: Member = None, deleter: Member = None) -> None:
        ...
        self.add_field(
            ...
            value='`original message not in cache; may be too old`',
            inline=False
        )
...
class DeleteLogEmbedFromMessage(Embed):
    def __init__(self, message: Message, deleter: Member) -> None:
        ...
        if message.attachments:
            self.add_field(name='ATTACHMENTS', value='\n'.join([
...
class MemberLeaveLogEmbed(_BaseMemberStateUpdateLogEmbed):
    def __init__(self, member: Member) -> None:
        ...
        self.add_field(
            name='join time', value=f'<t:{int(member.joined_at.timestamp())}:f>', inline=False)
        self.add_field(
            name='leave time',
            value=f'<t:{int(time())}:f>',
            inline=False)
        if member.roles[1:]:
            self.add_field(name='roles', value=' '.join(
...
class _MemberBanUpdateLogEmbed(Embed):
    def __init__(self, member: Member, audit_log: AuditLogEntry = None) -> None:
        ...
        self.add_field(
            name='reason',
            value=audit_log.reason or '`no reason provided`',
            inline=False
        )
