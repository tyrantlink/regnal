...
class auto_response_modal(Modal):
	...
	async def callback(self,interaction:Interaction) -> None:
		custom = await self.client.db.guilds.read(self.guild_id,['data','auto_responses','custom',self.method])
		...
		match len(self.children):
			case 1: # means is delete
				if trigger not in custom.keys():
					...
					return
				await self.client.db.guilds.unset(self.guild_id,['data','auto_responses','custom',self.method,trigger])
			...
			case 2: # means is add
				...
				await self.client.db.guilds.write(self.guild_id,['data','auto_responses','custom',self.method,trigger],au)
