...
class ConfigOptionTypeValidator(ConfigOptionTypeHint):
	...
	async def validate_string(self,value:str) -> str:
		...
		if self.option.attrs.options and self.option.attrs.enum is not None:
			try: value = self.option.attrs.enum[value]
			except KeyError: raise ConfigValidationError(f'invalid option `{value}`')
		return value
	...
	async def _validate_mentionable(self,value:GuildChannel|Role|User|Member) -> GuildChannel|Role|User|Member:
		return value
