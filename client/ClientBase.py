...
if TYPE_CHECKING:
    from extensions.auto_responses import AutoResponses
...
class ClientBase:
    ...
    async def _owner_init(self) -> None:
        self.owner_ids = {
            u['_id']
            async for u in
            self.db._client.users.find(
                {
                    'data.flags':
                    {
                        "$bitsAllSet": UserFlags.ADMIN
                    }
                },
                projection={'_id': True})
        }

        # set because Bot.owner_id is given
        self.owner_id = list(self.owner_ids)[0] if self.owner_ids else None
    ...
    async def on_ready(self) -> None:
        # ? i have to delay this to here because it's stupid or something
        self.helpers.load_commands()

        self.log.info(
            f'{self.user.name} ready in {round(perf_counter()-self._st,2)} seconds')
    ...
    async def on_application_command(self, ctx: ApplicationContext) -> None:
        self.log.info(
            f'{ctx.author.name} used {ctx.command.name}', ctx.guild_id)
    ...
    async def on_application_command_error(
        ...
        error: ApplicationCommandInvokeError
    ) -> None:
        if isinstance(error, CheckFailure):
            return
        ...
        embed = Embed(
            ...
            description='the issue has been automatically reported and should be fixed soon.',
            color=0xff6969)
        ...
        async with ClientSession() as session:
            wh = Webhook.from_url(
                self.project.webhooks.errors, session=session)
    ...
    async def on_guild_join(self, guild: Guild) -> GuildDocument:
        if (
            self.project.bot.guilds and
            guild.id not in [
                *self.project.config.base_guilds, *self.project.bot.guilds
            ]
        ):
            try:
                await guild.leave()
            except HTTPException:
                self.log.error(f'failed to leave guild {guild.id}')
    ...
    async def handle_guild_message(self, message: Message) -> None:
        ...
        if guild.attached_bot != self.user.id:
            if guild.attached_bot is not None:
                self.log.error(
                    f'guild {guild.name} ({guild.id}) is attached to a different bot ({guild.attached_bot})', guild.id
                )
        ...
        if day not in guild.data.activity.keys():
            while len(guild.data.activity.keys()) > 30:
                guild.data.activity.pop(
                    sorted(list(guild.data.activity.keys()))[0]
                )
                save_changes = False
