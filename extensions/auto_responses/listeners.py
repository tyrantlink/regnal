...
class ExtensionAutoResponsesListeners(ExtensionAutoResponsesSubCog):
    ...
    async def on_raw_reaction_add(self, payload: RawReactionActionEvent) -> None:
        ...
        if log is None or (triggerer := log.data.get('triggerer', None)) is None:
            return
    ...
    async def on_message(self, message: Message) -> None:
        ...
        if not user.config.general.auto_responses:
            return

        # ignore AU_BANNED users
        if user.data.flags & UserFlags.AUTO_RESPONSE_BANNED:
            return
        ...
        channel = (
            ...
            if isinstance(message.channel, Thread) else
            message.channel
        )
        ...
        if not args.force:
            match guild.config.auto_responses.enabled:
                case TWBFMode.false:
                    return
                case TWBFMode.whitelist if (
                    channel.id not in guild.data.auto_responses.whitelist
                ):
                    return
                case TWBFMode.blacklist if (
                    channel.id in guild.data.auto_responses.blacklist
                ):
                    return
                case TWBFMode.true | _:
                    pass
