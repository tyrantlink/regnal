...
class PermissionManagerView(SubView):
    ...
    def value_id(self) -> str:
        ...
        return (
            '@everyone'
            if self.selected_value == self.user.guild.default_role else
            str(self.selected_value.id)
        )
    ...
    async def reload_embed(self, guild_doc: GuildDoc | None = None) -> None:
        ...
        if self.selected_value is not None:
            permissions = '\n'.join(
                guild_doc.data.permissions.get(self.value_id, []))
    ...
    async def log(self, added: set | None = None, removed: set | None = None) -> None:
        ...
        if (
            ...
            channel = self.user.guild.get_channel(
                int(guild_doc.config.logging.channel))
    ...
    async def button_modify(self, button: Button, interaction: Interaction) -> None:
        ...
        for permission in new_value:
            ...
            permission_diff = self.client.permissions.matcher(
            ...
            if (
                ...
                await self.client.helpers.send_error(
                    interaction,
                    f'''
                    the matcher `{permission}` matches permissions that you don\'t have!
