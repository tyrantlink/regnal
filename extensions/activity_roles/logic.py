...
class ExtensionActivityRolesLogic(ExtensionActivityRolesSubCog):
    ...
    async def log_changes(self, guild: Guild, changes: ActivityRoleChanges) -> None:
        ...
        max_roles = guild_doc.config.activity_roles.max_roles

        embed.set_footer(
            text=f'day {guild_doc.get_current_day()} users: {changes.total_users}/{max_roles}'
        )
