...
class ExtensionQOTDLogic(ExtensionQOTDSubCog):
    async def reload_packs(self) -> None:
        ...
        for file in files:
            async with open(f'extensions/qotd/packs/{file}') as f:
                self.packs[file.removesuffix('.json')] = \
                    QOTDPack.model_validate_json(await f.read())
    ...
    async def find_guilds(self) -> AsyncIterator[tuple[Guild, GuildDocument]]:
        ...
        async for guild in self.client.db._client.guilds.find(
            ...
            if guild['_id'] not in _client_guilds or guild['_id'] in self.recently_asked:
                continue
            ...
            if guild_doc.data.qotd.last == guild_doc.get_current_day():
                continue
    ...
    async def log_ask_custom(self, author: Member, question: str) -> None:
        ...
        channel = (
            author.guild.get_channel(guild_doc.config.logging.channel) or
        ...
        embed.set_author(
            name=f'{author.display_name} asked a custom question!',
    ...
    async def get_question(self, guild_doc: GuildDocument) -> tuple[GuildDocument, Embed]:
        embed = Embed(
            ...
            color=await self.client.helpers.embed_color(guild_doc.id)
        )
        ...
        if guild_doc.data.qotd.nextup:
            ...
            embed.set_footer(
                text=f'custom question by {question.author}', icon_url=question.icon
            )
        ...
        if not guild_doc.data.qotd.packs:
            raise ValueError(
                f'no qotd packs available for guild {guild_doc.name} ({guild_doc.id})'
            )
        ...
        guild_doc.data.qotd.asked[pack] = f'{asked[:index]}1{asked[index+1:]}'

        return guild_doc, embed

    async def ask_qotd(self, guild: Guild, guild_doc: GuildDocument) -> None:
        channel = (
            guild.get_channel(guild_doc.config.qotd.channel) or
            await self.client.fetch_channel(guild_doc.config.qotd.channel)
        )

        if channel is None or channel.type != ChannelType.forum:
            return False

        _roles = [
            i
            for i in guild.roles
            if i.name.lower() == 'qotd' and
            not i.is_bot_managed()
        ]
        ...
        try:
            ...
            if (
                ...
                (
                    old_thread := channel.get_thread(thread_id)
                ) is not None
            ):
        ...
        try:
            thread = await channel.create_thread(
                name=question if len(
                    question) <= 100 else f'{question[:97]}...',
                content=ping_role,
                embed=embed,
