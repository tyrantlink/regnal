...
class ModMailConfirmationView(View):
        ...
        if self.source_message:
            self.message_proxy = Embed(
                ...
                description=(
                    ...
                    if len(self.source_message.content) < 1893 else
                    f'{self.source_message.content[:1893]}...'
                ),
                color=0xff6969
            )
            ...
            if self.source_message.attachments:
                self.message_proxy.add_field(
                    ...
                    value='\n'.join(
                        ...
                        for attachment in
                        self.source_message.attachments
                    )
                )
    ...
    def embeds(self) -> list[Embed]:
        return (
            [self.message_proxy, self.issue_embed]
            if self.source_message
            else [self.issue_embed]
        )
    ...
    async def _new_modmail_document(self, interaction: Interaction) -> ModMail:
        anonymous = (
            self.issue_embed.author.name == 'anonymous' and
            self.issue_embed.author.icon_url == self.client.user.display_avatar.url
        )

        last_modmail = await self.client.db._client.modmail.find_one(
            {'guild': interaction.guild_id},
            projection={'_id': False, 'modmail_id': True},
            sort=[('modmail_id', -1)]
        )
        ...
        mail = ModMail(
            ...
            title=self.title,
            message=(
                int(self.message_proxy.footer.text)
                if self.source_message else
                None
            )
        )
    ...
    async def new_thread(self, interaction: Interaction) -> str:
        ...
        channel: ForumChannel = \
            self.client.get_channel(guild_doc.config.modmail.channel)

        modmail = await self._new_modmail_document(interaction)
        self.issue_embed.set_footer(text=f'modmail id: {modmail.modmail_id}')
        name = "anonymous" if self.anonymous else self.reporter.name

        thread = await channel.create_thread(
            name=f'{modmail.modmail_id} - {name} - {self.title}',
            content=(
    ...
    async def button_send_report(self, button: Button, interaction: Interaction) -> None:
        ...
        embed = Embed(
            ...
            description=self.client.helpers.handle_cmd_ref(
                'you can check the status of your report using {cmd_ref[modmail]}'),
        ...
        if not interaction.user.can_send():
            await interaction.followup.send(
                embed=Embed(
                    title='warning!',
                    description='''
                        i cannot send you a DM to notify you of new messages, you may have to allow direct messages from a server i'm in.\n
    ...
    async def button_anonymous_mode(self, button: Button, interaction: Interaction) -> None:
        ...
        await interaction.response.edit_message(
            embeds=[self.title_embed, *self.embeds],
            view=self
        )
