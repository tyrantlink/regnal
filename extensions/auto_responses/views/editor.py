...
class AutoResponseEditorView(SubView):
    ...
    async def __ainit__(self) -> None:
        if self.override:
            ...
            self.au = self.au.with_overrides(
                guild_doc.data.auto_responses.overrides.get(self.au.id, {})
            )
    ...
    async def reload_embed(self) -> None:
        self.embed = Embed(
            title=(
                f'override auto response {self.au.id}'
        ...
        if not self.override:
            self.embed.add_field(
                ...
                value=self.client.helpers.handle_cmd_ref(
                    self.au.data.source) if self.au.data.source else None
            )
    ...
    async def reload_items(self) -> None:
        ...
        self.get_item(
            'button_trigger_response_source_weight_chance'
    ...
    async def button_trigger_response_source_weight_chance(self, button: Button, interaction: Interaction) -> None:
        ...
        try:
            ...
            fail_reasons.append('weight must be an number')

        try:
            float(chance)
            if not 0 <= float(chance) <= 100:
                fail_reasons.append('chance must be between 0 and 100')
        except ValueError:
            fail_reasons.append('chance must be a number')
    ...
    async def save_override(self, interaction: Interaction) -> None:
        ...
        # ? has to be hard save because messing with dictionaries doesn't count as a change
