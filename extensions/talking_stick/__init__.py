...
class ExtensionTalkingStick(Cog):
	...
	async def find_guilds(self) -> AsyncIterator[tuple[Guild,GuildDocument]]:
		...
		self._rescan = False
		self.client.log.debug(f'finding talking stick guilds took {perf_counter()-_st:.4f}s ({len(guilds)}/{len(_client_guilds)} guilds)')
	...
	async def talking_stick_loop(self) -> None:
		...
		async for guild,guild_doc in self.find_guilds():
			if guild.id != 844127424526680084: continue
