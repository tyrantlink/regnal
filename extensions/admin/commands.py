	...
	async def admin_filter(self,ctx:ApplicationContext,location:str,mode:str,filter:str|None,channel:TextChannel|None) -> None:
		...
		match mode:
			case 'add':
				...
				match location:
					case 'guild': await self.client.db.guilds.append(ctx.guild.id,['data','regex','guild'],filter)
					case 'channel': await self.client.db.guilds.append(ctx.guild.id,['data','regex','channel',str(channel.id)],filter)
			...
			case 'remove':
				...
				match location:
					case 'guild': await self.client.db.guilds.remove(ctx.guild.id,['data','regex','guild'],filter)
					case 'channel': await self.client.db.guilds.remove(ctx.guild.id,['data','regex','channel',str(channel.id)],filter)
					case _: await ctx.response.send_message('invalid location, how did you even do that?',ephemeral=await self.client.hide(ctx))
			...
			case 'list': 
				match location:
					case 'guild': res = await self.client.db.guilds.read(ctx.guild.id,['data','regex','guild'])
					case 'channel': res = await self.client.db.guilds.read(ctx.guild.id,['data','regex','channel',str(channel.id)])
					case _: res = []
