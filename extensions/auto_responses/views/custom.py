...
class CustomAutoResponseView(SubView):
	...
	async def reload_items(self) -> None:
		...
		if not self.au:
			...
			self.get_item('select_auto_response').placeholder = 'no auto responses found!'
		...
		self.get_item('select_auto_response').placeholder = 'select an auto response'
		self.get_item('select_auto_response').options = [
			SelectOption(
				label = au.trigger if len(au.trigger) < 50 else f'{au.trigger[:50]}...',
				description = au.response[:50] if len(au.response) < 50 else f'{au.response[:50]}...',
				value = au.id
			) for au in self.au]	
	...
	async def button_import_script(self,button:Button,interaction:Interaction) -> None:
		...
		guild_doc.data.auto_responses.imported_scripts = list(set(guild_doc.data.auto_responses.imported_scripts + [au_id]))
	...
	async def button_delete(self,button:Button,interaction:Interaction) -> None:
		...
		await self.reload_embed()
		await modal.interaction.response.edit_message(embed=self.embed,view=self)
		await interaction.followup.send(f'successfully deleted auto response {au_id}!',ephemeral=True)