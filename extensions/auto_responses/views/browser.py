...
class AutoResponseBrowserView(View):
    ...
    async def __ainit__(self) -> None:
        ...
        self.au_found_dict = {k: [] for k in [
            'base', 'custom', 'unique', 'mention', 'personal', 'script']
        }
        ...
        self.au_found = [
            au for category in self.au_found_dict.values()
            for au in category
        ]
    ...
    async def button_previous(self, button: Button, interaction: Interaction) -> None:
        ...
        if self.au_index < 0:
            self.au_index = len(self.au_found)-1
    ...
    async def button_next(self, button: Button, interaction: Interaction) -> None:
        ...
        if self.au_index >= len(self.au_found):
            self.au_index = 0
    ...
    async def button_with_overrides(self, button: Button, interaction: Interaction) -> None:
        ...
        self.au = (
            self.au.with_overrides(
                self.guild_overrides.get(self.au.id, {})
            )
            if self.with_overrides else
            self.client.au.get(self.au_found[self.au_index])
        )
