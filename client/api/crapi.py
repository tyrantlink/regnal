...
class CrAPI(MessageHandler):
    def __init__(self, client: 'Client') -> None:
        ...
        self.session = ClientSession(
            self.base_url, headers={'token': self.token})
    ...
    async def connect(self, reconnect: bool = True) -> None:
        ...
        if reconnect:
            create_task(self._reconnection_handler())
    ...
    async def _heartbeat(self) -> None:
        while True:
            ...
            try:
                await self.gateway_send(Heartbeat())
            except ValueError:
                break

    async def gateway_send(self, message: BaseGatewayMessage) -> None:
        if not self.is_connected:
            raise ValueError('not connected to crAPI gateway!')
