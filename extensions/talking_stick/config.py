...
async def validate_channel(
	...
	if value.type != ChannelType.text: raise ConfigValidationError('channel must be a text channel!')
	return value,None if value.can_send() else (
		'i can\'t send messages in that channel, config still saved, but please correct this!')
...
def register_config(config:'Config') -> None:
	...
	config.register_option(
		'guild','talking_stick',ConfigOption(
		name = 'enabled',type = OptionType.BOOL,default = False,
		short_description = 'enable/disable talking stick',
		description= '''daily random roll to give an active user a specific role\n
		intended to give users send_messages permissions in a channel, but can be used for anything'''.replace('\t','')))

	config.register_option('guild','talking_stick',ConfigOption(
		name = 'channel',type = OptionType.CHANNEL,default = None,
		attrs = ConfigAttrs(validation = validate_channel),
		short_description = 'channel used to announce the talking stick',
		description= '''channel used to announce the talking stick'''.replace('\t','')))

	config.register_option('guild','talking_stick',ConfigOption(
		name = 'role',type = OptionType.ROLE,default = None,
		attrs = ConfigAttrs(validation = validate_role),
		short_description = 'role given to the user',
		description= '''role given to the user'''.replace('\t','')))

	config.register_option('guild','talking_stick',ConfigOption(
		name = 'limit',type = OptionType.ROLE,default = None,
		...
		if not set, all users can get the talking stick'''.replace('\t','')))
	...
	config.register_option('guild','talking_stick',ConfigOption(
		...
		follows guild set timezone'''.replace('\t','')))

	config.register_option('guild','talking_stick',ConfigOption(
		name = 'announcement_message',type = OptionType.STRING,default = 'congrats {user} you have the talking stick.',
		attrs = ConfigAttrs(max_length = 200),
		...
		format: {user} is replaced with the user's mention'''.replace('\t','')))
