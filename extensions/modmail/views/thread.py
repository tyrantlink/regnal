...
class ModMailThreadView(SubView):
    ...
    async def reload(self, force_last_page: bool = False) -> None:
        ...
        if self.guild is None:
            try:
                self.guild = (
                    self.client.get_guild(self.modmail.guild) or
                    await self.client.fetch_guild(self.modmail.guild)
                )
            except Forbidden:
                self.guild = None
        ...
        self.add_items(
            ...
            self.button_next,
            self.button_new_message,
            self.button_close
        )

        self.pages = (
            len(self.modmail.messages)//5) - (
                0 if len(self.modmail.messages) % 5 else 1
        )
        ...
        user_doc.data.modmail_threads[self.modmail_id] = len(
            self.modmail.messages
        )
        ...
        for message in self.modmail.messages[5*self.page:5*(self.page+1)]:
            ...
            embed.set_author(
                name=(
                    ...
                    'anonymous'
                    if self.guild else
                    'must be in the correct server to see author'),
    ...
    async def button_close(self, button: Button, interaction: Interaction) -> None:
        if not self.close_confirm:
            ...
            await interaction.followup.send(
                'please press the button again to confirm\nonce the thread is closed no more messages can be sent',
                ephemeral=True)
            return
